...
| v17 | **Axe imprime sur mesure : rivet snap-fit (`pin_mode = "rivet"`, defaut)** | pas de quincaillerie : l'axe est une piece imprimee (`part="pin"`). Pour une charniere l'axe est juste un essieu CAPTURE (A et B tournent libre autour, alesages tous en jeu) : collerette qui bute dessus + barbe fendue qui clipse dessous, se repince pour retirer. `pin_mode = "m4"` garde la vis M4 capuchonnee en repli. Attention : axe plastique -> plus faible en flexion qu'une vis acier ; imprimer couche/45 deg pour la tenue |
